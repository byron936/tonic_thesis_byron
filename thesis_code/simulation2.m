...
ue_positions = ue_tbl(:, 2:5);     % [cell, X, Y, Z]
...
function [best_thetakSi] = optimize_SSB_periodicity(Si, U, PkSi, ue_positions, satellite_positions, K, M, POP_SIZE, GA_GEN, slots_per_epoch, cell_tbl)
    ...
    for gen = 1:GA_GEN
        ...
        for i = 1:POP_SIZE
            ...
            [totalalphaSi, ~] = calculate_alphauSi(Si, U, thetakSi, PkSi, ue_positions, satellite_positions, slots_per_epoch, cell_tbl);
        ...
        population = select_next_generation(population, offspring, fitness, Si, U, PkSi, ue_positions, satellite_positions, slots_per_epoch, cell_tbl);
    end
...
function next_population = select_next_generation(population, offspring, fitness, Si, U, PkSi, ue_positions, satellite_positions, slots_per_epoch, cell_tbl)
    ...
    for i = 1:size(offspring,2)
        ...
        [totalalphaSi,~] = calculate_alphauSi(Si, U, individual, PkSi, ue_positions, satellite_positions, slots_per_epoch, cell_tbl); % 必需補齊你的輸入
...
function [best_P] = optimize_power_allocation(Si, U, thetakSi, ue_positions, satellite_positions, slots_per_epoch, K, P_total, T_max, T_min, cooling_rate, cell_tbl)
    % Initialize power allocation Pcurrent with feasible solution
    Pcurrent = ones(K, 1) * (P_total / K); % Equal power initial guess
    [totalalphaSi, ~] = calculate_alphauSi(Si, U, thetakSi, Pcurrent, ue_positions, satellite_positions, slots_per_epoch, cell_tbl);
    current_delay = totalalphaSi / U;
    ...
    while T > T_min
        ...
        [totalalphaSi, ~] = calculate_alphauSi(Si, U, thetakSi, Pnew, ue_positions, satellite_positions, slots_per_epoch, cell_tbl);
        new_delay = totalalphaSi / U;
...
function P_new = perturb_power_allocation(P_current, P_total)
    ...
    delta = 0.1 * P_total * (2*rand - 1); % random between -0.1*P_total to +0.1*P_total
